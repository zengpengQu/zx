 ...
 */
@Entity
@Table(name="DM_CIM_VIP_PERSON_INTRO",schema = "SCH_DUTY")
public class VipPersonIntro extends IdEntity implements Jsonable, Serializable {

    private static final Logger LOG = LoggerFactory.getLogger(VipPersonIntro.class);

    public VipPersonIntro() {}
    ...
    public NationConditionManagement getNationConditionManagement() {
        if(nationConditionManagement != null) {
            try {
                ...
                LOG.error("获取关联对象NationConditionManagement错误", e);
                return null;
            }
        }
    ...
    public JSON getJsonObject() {
        ...
        jsonObject.put("personIntro", personIntro);
