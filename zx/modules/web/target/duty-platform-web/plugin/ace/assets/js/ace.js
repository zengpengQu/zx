...
jQuery(function($) {
	...
	function enableDemoAjax() {		
        ...
        var demo_ajax_options = {     //这个异步加载专为最新动态用
            'close_active': true,

            //'default_url': 'page/index/main',//default hash
            ...
            'content_url': function(hash) {
                ...
                if(param=="")
                {
                    param="?rd="+Math.random();
                }else{
                    param = param+"&rd="+Math.random();
                }

                //for example in Ace HTML demo version we convert /ajax/index.html#page/gallery to > /ajax/content/gallery.html and load it
                if(path.match(/(\/)(getParamNewStatus\.do)?/))
                {
                    path = path.replace(/(\/)(getParamNewStatus\.do)?/, '/'+hash.replace(/^page\//, '')+'.do' + param) ;
                    //alert(path + " $ " + param)
                    return path;
                }
