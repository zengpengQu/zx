...
<div class="row">
    <div class="col-xs-12">
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="header-table-th">礼品出库管理列表</div>
...
<div id="result-dialog" class="hide">
...
<script id="template-result" type="text/x-handlebars-template">
    <table class="table table-striped table-bordered table-hover center">
        ...
        <tr>
            ...
            <th class="center" width="30%">礼品名称</th>
            <th class="center" width="60%">出库结果</th>
        </tr>
        </thead>
        <tbody>
        {{#each dataList}}
...
<script type="text/javascript">
    ...
    $('.page-content-area').ace_ajax('loadScripts', scripts, function () {
        ...
        var loadUrl = '${basePath}/giftOutbound/load-outbound-list.do';
        var editUrl = '#page/giftOutbound/load-outbound-edit';
        var viewUrl = '#page/giftOutbound/load-outbound-view';
        var deleteUrl = '${basePath}/giftOutbound/load-outbound-delete.do';

        var colModel = [
            {name: 'id', index: 'id', hidden: true},
            {name: 'outboundCode', header: '出库编码', index: 'outboundCode', sortable: false},
            ...
            {name: 'outboundTime', header: '出库时间', index: 'outboundTime', sortable: false},
            {name: 'isOutbound', header: '出库状态', index: 'isOutbound', sortable: false, align: 'center', hidden: true},
            {
                header: '出库状态',
                sortable: false,
                editable: false,
                width: 100,
                formatter: outboundStateFormatter
            },
            {
                name: 'basic',
                header: '基本操作',
        ...
        function outboundStateFormatter(cellvalue, options, rowData, action) {
            ...
            var operations = [];
            if (isOutbound == '未出库') {
                operations.push("<span class='label label-success arrowed arrowed-right'>" + '未出库' + "</span>");
            } else if (isOutbound == '已出库') {
                operations.push("<span class='label label-info arrowed arrowed-right'>" + '已出库' + "</span>");
            }
